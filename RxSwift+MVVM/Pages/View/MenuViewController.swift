    ...
    func setupBindings() {
        ...
        viewModel.showOrderPage
            .subscribe(onNext: { [weak self] selectedMenus in
                self?.performSegue(withIdentifier: OrderViewController.identifier, sender: selectedMenus)
            })
            .disposed(by: disposeBag)
        ...
        viewModel.activated
            ...
            .bind(to: activityIndicator.rx.isHidden)
                ...
                // 테이블뷰 아이템들
                viewModel.allMenus
                .bind(to: tableView.rx.items(cellIdentifier: MenuItemTableViewCell.identifier, cellType: MenuItemTableViewCell.self)) {
                    _, item, cell in
        ...
        // 선택된 아이템 총개수
        viewModel.totoalSelectedCountText
